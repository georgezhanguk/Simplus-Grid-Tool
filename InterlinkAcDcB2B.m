% Model of an ac-dc converter for inter-linking ac and dc grids.

% Author(s): Yitong Li Yaoyu Hu

%% Notes
%
% The model is in 
% ac-side: load convention, admittance form.
% dc-side: load convention, admittance form.
%
% The model is simply a three phase voltage source inverter, rather than a
% back-to-back topology.

%% Class

classdef InterlinkAcDcB2B < SimplusGT.Class.ModelAdvance
    
    % For temporary use
    properties(Access = protected)
        i_q_r;
    end
    
    methods
        % constructor
        function obj = InterlinkAcDc(varargin)
            % Support name-value pair arguments when constructing object
            setProperties(obj,nargin,varargin{:});
        end
    end

    methods(Static)
        
        function [State,Input,Output] = SignalList(obj)
          	% Notes:
            %
            % The first three inputs must be v_d, v_q, and v; and the first
            % three outputs must be i_d, i_q, and i.
            %
            % v_d, v_q, i_d, i_q are ac-grid electrical port.v, i are the
            % dc-grid electrical port.
            %
            % v_dc is the dc link voltage, there is an inductor between v
            % and v_dc. This inductor makes the system admittance model
            % proper seen from the dc side.
            % Control schemes：Apparatus 2000&2001 → GFL
            %                  Apparatus 2002 → FFL-VFM


            State = {'i_d','i_q','i_d_i','i_q_i','v_d_i','v_q_i','theta','v_dc','v_dc_i','i','i_i','v_lk','v_lk_i','p_sum_i','p_lk_i'};
        	Input = {'v_d','v_q','v','ang_r'};
            Output = {'i_d','i_q','i','w','v_dc','theta'};
        end
        
        function [x_e,u_e,xi] = Equilibrium(obj)
            % Get the power PowerFlow values
            % The interlink converter has two sets of power flow results
            % because it is connected to two buses: the first one is ac,
            % and the second one is dc.
            % AC PF Values
            P_ac    = obj.PowerFlow(1);
            Q_ac    = obj.PowerFlow(2);
            Vg_ac   = obj.PowerFlow(3);
            xi      = obj.PowerFlow(4);
            w       = obj.PowerFlow(5);
            % DC PF Values
            P_dc    = obj.PowerFlow(6);
            Vg_dc   = obj.PowerFlow(8);
            
            % Get parameters
            wL_ac = obj.Para(2);
            W0 = obj.Para(13);
            L_ac  = wL_ac/W0;
            R_ac  = obj.Para(3);
            R_dc  = obj.Para(5);

            % Calculate paramters
            i_d = P_ac/Vg_ac;
            i_q = -Q_ac/Vg_ac;     % Because of conjugate "i"
            v_d = Vg_ac;
            v_q = 0;
            i_dq = i_d + 1j*i_q;
            v_dq = v_d + 1j*v_q;
            e_dq = v_dq - i_dq * (R_ac + 1j*L_ac*w);
            e_d = real(e_dq);
            e_q = imag(e_dq);
            i_d_i = e_d;
            i_q_i = e_q;
            v_d_i = -i_d;
            v_q_i = -i_q;
            i_d_r = i_d;
            i_q_r = i_q;
            w_pll_i = w;
            v_dc_i = i_d_r;

            theta = xi;
            
            v = Vg_dc;
            i = P_dc/Vg_dc;
            
            v_dc = Vg_dc - i*R_dc;
            
            ang_r = 0;
            
            % ??? Temp
            obj.i_q_r = i_q_r;

            % Get equilibrium

            	x_e = [i_d; i_q; i_d_i; i_q_i; v_d_i; v_q_i; theta; v_dc; v_dc_i; i; i_i; v_lk; p_sum_i; p_lk_i];
            	u_e = [v_d; v_q; v; ang_r]; 

                   
        end

        function [Output] = StateSpaceEqu(obj,x,u,CallFlag)
            % Get power flow
            P_ac    = obj.PowerFlow(1);
            Vg_ac   = obj.PowerFlow(3);
            Vg_dc   = obj.PowerFlow(8);

           	% Get parameters
            xC_dc      = obj.Para(1);
            xwL_ac     = obj.Para(2);
            xR_ac      = obj.Para(3);
            xwL_dc     = obj.Para(4);
            xR_dc      = obj.Para(5);
            xfidq      = obj.Para(6);
            xfvdc      = obj.Para(7);
            xfpll      = obj.Para(8);
            R          = obj.Para(9);
            K          = obj.Para(10);
            N          = obj.Para(11);
            v_dc_ref   = obj.Para(12);
            W0         = obj.Para(13);
            if obj.ApparatusType==2004
                %xfvdc      = obj.Para(14);
                xfvlk      = obj.Para(15);
            end
            V_dc = 1;
            w_vdc = xfvdc*2*pi; 	% (rad/s) bandwidth, vdc
            w_pll     = xfpll*2*pi;  	% (rad/s) bandwidth, pll
            w_i     = xfidq*2*pi; 	% (rad/s) bandwidth, idq
            w_tau_pll = 200*2*pi;   % (rad/s) PLL filter bandwidth
            
            L_ac    = xwL_ac/W0;
            R_ac    = xR_ac;
            L_dc    = xwL_dc/W0;
            R_dc    = xR_dc;
            C_dc    = xC_dc;
            kp_v_dc = V_dc*xC_dc*w_vdc;
            ki_v_dc = kp_v_dc*w_vdc/4;
            kp_i_dq = L_ac * w_i;
            ki_i_dq = L_ac * w_i^2 /4;
            kp_v_dq = 1/(16*w_i*L_ac);
            ki_v_dq = 1/(4*L_ac);
            kp_v_lk = 
            ki_v_lk = 
            D_dc    =
            D_ac    =
            kp_pll = w_pll;
            ki_pll = kp_pll * w_pll/4;
            tau_pll = 1/w_tau_pll;

            if obj.ApparatusType==2005
                xfvdq = 300;
                w_v_dq = xfvdq*2*pi;
                w_i_dq = xfidq*2*pi;
            
                kp_i_dq = w_i_dq*L_ac;
                ki_i_dq = w_i_dq^2*L_ac/4;
                Cf = 0.02/W0;
                kp_v_dq = w_v_dq*Cf;
                ki_v_dq = w_v_dq^2*Cf/4*50;
            end
            % Get states


                i_d     = x(1); 
                i_q     = x(2);
                i_d_i   = x(3);
                i_q_i   = x(4);
                v_d_i   = x(5);
                v_q_i   = x(6);
                theta   = x(7);
                v_dc    = x(8);
                v_dc_i  = x(9);
                i       = x(10);
                i_i     = x(11);
                v_lk    = x(11);
                p_sum_i = x(12);
                p_lk_i  = x(13);

   
            % Get input
            v_d    = u(1);
            v_q    = u(2);
            v      = u(3);
            ang_r  = u(4);

            % State space equations
            % dx/dt = f(x,u)
            % y     = g(x,u)

            % ### Call state equation: dx/dt = f(x,u)

                f_ac_nom = W0/2/pi;  
                v_dc_nom = 1;
                v_d_r = 1;
                v_q_r = 0;

                % Linking Capacitor
                i_dc_lk = i*v_dc/v_lk;
                i_ac_lk  = (e_d*i_d + e_q*i_q)/v_lk;
                i_c_lk = i_ac_lk + i_dc_lk;
                dv_lk = i_c_lk/C_lk;
                % Power Balance Control
                error_p_sum = (v - v_dc_nom) - B*(f - f_ac_nom);
                p_sum_ref = error_p_sum * kp_sum + p_sum_i;
                dp_sum_i = error_p_sum * ki_sum;
                error_p_lk = v_lk - v_lk_nom;
                p_lk_ref = error_p_lk * kp_lk + p_lk_i;
                dp_lk_i = error_p_lk * ki_lk;
                p_dc_ref = p_lk_ref - p_sum_ref;
                p_ac_ref = p_lk_ref + p_sum_ref;

                % P-Vdc Droop 
                v_dc_ref = (p_dc_ref - p_dc)*D_dc +  v_dc_nom;
                % P-f Droop
                f_ac_ref = (p_ac_ref - p_ac)*D_ac +  f_ac_nom;           

                % AC Voltage Control
                error_v_d = v_d_r - v_d;
              	error_v_q = v_q_r - v_q;
                dv_d_i = error_v_d*ki_v_dq;
                dv_q_i = error_v_q*ki_v_dq;
                i_d_r = -(error_v_d*kp_v_dq + v_d_i);
                i_q_r = -(error_v_q*kp_v_dq + v_q_i);

                % AC Current Control
                error_i_d = i_d_r-i_d;
                error_i_q = i_q_r-i_q;
                di_d_i = -error_i_d*ki_i_dq;
                di_q_i = -error_i_l*ki_i_dq;
                e_d = -error_i_d*kp_i_dq + i_d_i;
                e_q = -error_i_q*kp_i_dq + i_q_i;
 
                dtheta = 2*pi*f_ac_ref;  


                % DC Control
                dv_dc_i = (v_dc_ref - v_dc) * k_i_vdc;
                i_ref = (v_dc_ref - v_dc) * k_p_vdc + v_dc_i;
                di_i = (i_ref - i_dc_lk) * ki_lk;
                v_lk = (i_dc_ref - i_dc_lk) * kp_lk + i_i;

                % Ac-side L filter
                di_d = (v_d - R_ac*i_d + w*L_ac*i_q - e_d)/L_ac;
                di_q = (v_q - R_ac*i_q - w*L_ac*i_d - e_q)/L_ac;

                % Dc-side inductor
                d_i = (v - v_dc - R_dc*i)/L_dc;


            if CallFlag == 1
                    f_xu = [di_d; di_q; di_d_i; di_q_i; dv_d_i; dv_q_i; dtheta; dv_dc; dv_dc_i; di; di_i, dv_lk, dp_sum_i, dp_lk_i];
                    Output = f_xu;
            elseif CallFlag == 2
              	% ### Call output equation: y = g(x,u)
                g_xu = [i_d; i_q; i; w; v_dc; theta];
                Output = g_xu;
            end
        end
    end

end     % End class definition